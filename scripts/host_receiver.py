        ...
        try:
            ...
            while True:
                ...
                if not raw_byte:
                    ...
                    continue
                if raw_byte[0] == SOF_MSB:
                    raw_byte = self.ser.read(1)
                    if raw_byte and raw_byte[0] == SOF_LSB:
                        # 2. Read length and payload
                        length_byte = self.ser.read(1)
                        if not length_byte:
                            print("Incomplete packet received. Retrying...")
                            continue
                        length = length_byte[0]
                        payload = self.ser.read(length)
                        if len(payload) != length:
                            print("Incomplete payload received. Retrying...")
                            continue
                        received_crc_byte = self.ser.read(1)
                        if not received_crc_byte:
                            print("Incomplete CRC received. Retrying...")
                            continue
                        received_crc = received_crc_byte[0]
...
if __name__ == "__main__":
    if len(sys.argv) > 1 and sys.argv[1] == "--save-binary":
        save_binary = True
        print("Binary saving enabled: Received entropy will be saved to 'quantum_entropy.bin'")
    else:
        save_binary = False
    app = QApplication(sys.argv)
    vizualizer = StatsVisualizer()
    vizualizer.show()
